...
function run() {
  git fetch --tags
  cp -f ${PROJECT_ROOT_DIR}/utils/openssl_3_6_0_offset.c ${OPENSSL_DIR}/offset.c
  declare -A sslVerMap=()
  sslVerMap["0"]="0"
  ...
  for ver in ${!sslVerMap[@]}; do
    tag="openssl-3.6.${ver}"
    val=${sslVerMap[$ver]}
    header_file="${OUTPUT_DIR}/openssl_3_6_${val}_kern.c"
    header_define="OPENSSL_3_6_$(echo ${val} | tr "[:lower:]" "[:upper:]")_KERN_H"
